...
- When used with transactions, the `put` and `create` methods now return the item provided to DynamoDB by default, similar to using `put` and `create` on an entity directly.
