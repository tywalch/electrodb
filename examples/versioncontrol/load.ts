...
function createMockData() {
    ...
    const pullRequestComments = pullRequests.map((issue) => {
        ...
        return models.createMockPullRequestComment({ repoName, repoOwner, username });
    });

    const subscriptions = faker.helpers.arrayElements([ issues, pullRequests ]).flat().map(ticket => {
...
async function loadTable(options: LoadTableOptions) {
    ...
    await models.PullRequestComment.put(options.pullRequestComments).go({ concurrency: 3 });
