...
function toSortKey(indexDefinition: NonSpecificEntityIndex['sk']): IndexKey | undefined {
...
function toPartitionKey(indexDefinition: NonSpecificEntityIndex['pk']): IndexKey {
    return {
        field: indexDefinition.field,
        type: toIndexKeyType(indexDefinition.cast),
    }
...
export function createTableIndexDefinition(indexDefinition: NonSpecificEntityIndex): TableIndexDefinition {
    return {
        type: 'TableIndex',
...
export function createSecondaryIndexDefinition(indexDefinition: NonSpecificEntityIndex): SecondaryIndexDefinition {
    return {
        type: 'GlobalSecondaryIndex',
...
export function createLocalSecondaryIndexDefinition(indexDefinition: NonSpecificEntityIndex): LocalSecondaryIndexDefinition {
...
export function getIndexes(schema: NonSpecificSchema): IndexDefinition[] {
    ...
    for (const accessPattern in schema.indexes) {
        const indexDefinition = schema.indexes[accessPattern];
        if (indexDefinition.index === undefined && accessPattern === tableIndexName) {
