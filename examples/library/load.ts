...
import { createMockAuthor, createMockMember, createMockBook, createMockGenre, Genre, Member, Author, Book, BookItem, MemberItem, GenreItem, AuthorItem } from './models';
...
function createMockData() {
    ...
    const genres = books.flatMap((book) => {
        ...
        const author = authors.find(author => {
            return author.authorLastName === authorLastName && author.authorFirstName === authorLastName;
        });
        ...
        if (author) {
            author.genres = uniqueItems(uniqueGenres, 'genre').map((items) => items.genre);
        }
