...
import { library, Author, Book, Genre, Member, BOOK_IS_AVAILABLE, BookItem } from "./models";
...
async function getCopiesByAuthor(options: GetCopiesByAuthorOptions) {
    ...
    return Book.query
        .author({ authorFirstName, authorLastName, cursor })
        .go();
}
...
async function getBooksByAuthor(options: GetBooksByAuthorOptions) {
    ...
    const copies = await getCopiesByAuthor({ authorFirstName, authorLastName, cursor });
...
async function getAccountInfo(memberId: string) {
    const account = await library.collections
        .account({ memberId })
        .go()
...
async function getAvailableCopies(authorLastName: string, authorFirstName: string, bookId: string) {
    return Book.query
        .author({
            authorLastName,
            authorFirstName,
...
async function getLoanedCopies(authorLastName: string, authorFirstName: string, bookId: string) {
    return Book.query
        .author({
            authorLastName,
            authorFirstName,
...
async function getEditions(bookTitle: string, earliestReleaseDate: string, latestReleaseDate?: string) {
...
async function getCopiesByGenre(options: GetCopiesByGenreOptions) {
    ...
    const bookGenres = await Genre.query.categories({ genre, subgenre }).go({ cursor });
    const uniqueBookTitles = Array.from(new Set(bookGenres.data.map((book) => book.bookTitle)));
    const copies: BookItem[] = [];
    await Promise.all(uniqueBookTitles.map(async (bookTitle) => {
...
async function getBooksByAuthorAndGenre(options: GetBooksByAuthorAndGenreOptions) {
    const { cursor, genre, authorLastName, authorFirstName, subgenre } = options;
    const bookGenres = await Genre.query.author({
        authorFirstName,
        authorLastName,
        genre,
        subgenre,
