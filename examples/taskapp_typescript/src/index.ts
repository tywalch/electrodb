...
// used for loading values into dynamodb to query on (not required)
import Loader from "../../taskapp/lib/loader";

// Types in native TypeScript
import taskr from "./taskr";

// Useful types
import {EntityItem, CreateEntityItem} from "../../../";
...
async function query() {
  ...
  function createNewTask(item: CreateEntityItem<typeof taskr.entities.tasks>) {
    return taskr.entities.tasks.put(item).go();
  }

  // EntityItem< typeof YOUR_ENTITY > is exported to help you type items as they are stored/retrieved from DynamoDB
  function formatItem(item: EntityItem<typeof taskr.entities.tasks>) {
    //
  }
}
