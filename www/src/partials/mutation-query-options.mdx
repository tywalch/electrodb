...
```typescript
{
  ...
  returnOnConditionCheckFailure?: boolean | "all_old";
...
| returnOnConditionCheckFailure | _none_ | By default, a failed condition expression throws. Set to `true` to return `{ rejected: true }` instead, or `"all_old"` to also include the existing item under `data`. See [Handling Condition Check Failures](/en/mutations/conditions#handling-condition-check-failures). |
