...
> The **Entities** and **Table Definition** tabs show the setup this example runs against, and the generated parameters below are produced live by executing the example.

<LiveExample name="indexes-scope-without-scope" />
...
<LiveExample name="indexes-scope-with-scope" />
...
ElectroDB will accept a `get` request like this (note the pre/postfix on `accountId` in the generated parameters below):

> ElectroDB defaults keys to lowercase, though this can be configured using [Index Casing](#index-casing). The **Entity** and **Table Definition** tabs show the setup this example runs against, and the generated parameters below are produced live by executing the example.

<LiveExample name="indexes-attributes-as-indexes-get" />
...
> The **Entity** and **Table Definition** tabs show the setup this example runs against, and the generated parameters (including the `KeyConditionExpression`) below are produced live by executing the example.

<LiveExample name="indexes-multi-attribute-basic-query" />

#### Sort Key Operations

Sort key operations apply to the sk composite attributes in order. Preceding sk attributes are matched with equality, and the operation applies to the last provided attribute. Note that the pk composite attributes (`country`, `region`, and `city`) must always be fully provided to the `location` Access Pattern method itself; the `begins`, `gt`, and `between` operations below apply only to the sk composite attributes (`manufacturer`, `model`, `count`):

Begins with substring expression on 'manufacturer'

<LiveExample name="indexes-multi-attribute-begins" />

Greater than on 'count' (manufacturer and model matched with equality)

<LiveExample name="indexes-multi-attribute-gt" />

Between on 'count' (manufacturer and model matched with equality)

<LiveExample name="indexes-multi-attribute-between" />
...
Query across both entities via the shared collection. The **Entities**, **Service**, and **Table Definition** tabs show the setup this example runs against, and the generated parameters below are produced live by executing the example.

<LiveExample name="indexes-multi-attribute-collection-query" />

Collection queries also support sort key operations

<LiveExample name="indexes-multi-attribute-collection-begins" />
