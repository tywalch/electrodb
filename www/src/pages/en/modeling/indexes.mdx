...
```typescript
// begins_with on 'manufacturer' (city matched with equality)
...
### Pagination with Multi-Attribute Indexes

Pagination on composite indexes works transparently. Cursors returned by queries can be used to resume pagination as usual:
...
do {
  const response = await InventoryItem.query
    .location({ country: "US", region: "Georgia", city: "Atlanta" })
    .go({ cursor, limit: 25 });
...
The `createConversions` utility also supports composite indexes for converting between items, keys, and cursors:

```typescript
import { createConversions } from "electrodb";

const conversions = createConversions(InventoryItem);

// Convert an item to a cursor for the 'location' access pattern
const cursor = conversions.byAccessPattern.location.fromComposite.toCursor(item);

// Convert a cursor back to composite attributes
const composite = conversions.byAccessPattern.location.fromCursor.toComposite(cursor);
```
...
5. **No collections**: `composite` indexes do not support collections because these indexes use attribute values directly.

### Gotchas and Tips

- Query comparison operations (e.g. `gt`, `gte`, `lt`, `lte`, `between`) on `composite` indexes apply only to the last provided sort key attribute. This is different from `clustered` or `isolated` indexes where the comparison applies to all provided sort key attributes. This is because `clustered` and `isolated` indexes use concatenated string sort key values, which can be compared across all attributes. `composite` indexes use attribute values directly, and dynamodb only allows comparison applies to the last provided attribute. Be mindful of this, as results may vary between different index types.

- Best practice when using `clustered` or `isolated` indexes is to use generic index field names for you Partition and Sort Keys. This keeps the indexes flexible and reusable across different entities. The same is true for `composite` indexes, where the fields used to compose an index can also be generisized. To support this with ElectroDB, use the `field` property on individual attributes to map to the field name on the DynamoDB table KeySchema. 
