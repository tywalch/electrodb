---
title: Collections
description: Creating entity entity relationships through collections
keywords:
  - electrodb
  - docs
  - concepts
...
> ElectroDB Collections use a single DynamoDB query to retrieve results. One query is made to retrieve results for all Entities (the benefits of single table design), however keep in mind that DynamoDB returns all records in order of the Entity's sort key. In cases where your partition contains a large volume of items, it is possible some entities will not return items during pagination. This can be mitigated through the use of [Index Types](/latest/modeling/indexes#index-types).
...
To query across entities, collection queries make use of ElectroDB's Sort Key structure, which prefixes Sort Key fields with the collection name. Unlike an Entity Query, Collection queries for [isolated indexes](/latest/modeling/indexes#isolated-indexes) only leverage [Composite Attributes](/latest/modeling/indexes#composing-attributes) from an access pattern's Partition Key, while Collection queries for [clustered indexes](/latest/modeling/indexes#clustered-indexes) allow you to query on both Partition and Sort Keys.
...
Unlike Entity Queries which return an array, Collection Queries return an object. This object will have a key for every Entity name (or [Entity Alias](/latest/modeling/services#creation)) associated with that Collection, and an array for all results queried that belong to that Entity.
...
Sub-Collections are an extension of [Collection](/latest/modeling/collections) functionality that allow you to model more advanced access patterns. Collections and Sub-Collections are defined on [Indexes](/latest/modeling/indexes) via a property called `collection`, as either a string or string array respectively.

> Sub-Collections are only supported on ["isolated" index](/latest/modeling/indexes#isolated-indexes) types.
...
> Please refer to the Entities defined in the section [Sub-Collections](/latest/modeling/indexes#sub-collections) as the source of examples within this section.

### Index Naming Conventions

The following is an access pattern on the "employees" entity defined [here](/latest/modeling/#sub-collections):
