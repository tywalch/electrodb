...
When joining a Entity to a Service, ElectroDB will perform numerous validations to ensure the provided Entities are consistently defined. The following is a list of the validations performed on Service creation:

- [Entity](/latest/modeling/entities) names must be unique across a Service.
- [Collection](/latest/modeling/collections) names must be unique across a Service.
- All [Collections](/latest/modeling/collections) map to on the same DynamoDB indexes with the same index field names. See [Indexes](/latest/modeling/indexes).
- Partition Key [Composite Attributes](#composite attribute-arrays) on a [Collection](/latest/modeling/collections) must have the same attribute names and labels (if applicable). See [Attribute Definitions](/latest/modeling/attributes#attribute-definition).
- The [name of the Service in the Model](/latest/modeling/entity#entity-definition-schema) must match the Name defined on the [Service](/latest/modeling/services) instance.
- If the attributes of an Entity have overlapping names with other attributes in that service, they must all have compatible or matching [attribute definitions](/latest/modeling/attributes#attribute-definition).