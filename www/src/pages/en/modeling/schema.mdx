...
> ElectroDB is a strongly typed library, and makes use of TypeScript's more advanced type inference functionality. For this reason, Schemas are typically defined directly in the Entity's constructor. If you do need to define a schema outside of a constructor, it is recommended to define your schema variable with an `as const` assertion ([read more here](https://www.typescriptlang.org/docs/handbook/release-notes/typescript-3-4.html#const-assertions)) to ensure TypeScript is able to infer the your schema correctly.
