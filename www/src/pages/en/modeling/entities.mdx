...
| client   | (optional) A DynamoDB document client (e.g. `DynamoDBDocumentClient` from `@aws-sdk/lib-dynamodb`) for use when querying a DynamoDB table. This is optional if you wish to only use the `params` functionality, or if you intend to provide the client dynamically at query time via [execution options](/en/core-concepts/executing-queries#execution-options). |
