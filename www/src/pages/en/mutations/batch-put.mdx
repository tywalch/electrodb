...
> Performing a Batch Put will return an array of "unprocessed" records. An empty array signifies all records returned were processed. If you want the raw DynamoDB response you can always use the option `{ data: "raw" }`, more detail found here: [Execution Options](#execution-options).
...
<LiveExample name="batch-put-store-locations" />
