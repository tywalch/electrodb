...
In DynamoDB, `update` operations will create an item by default if record being updated does not exist. Alternatively, the `patch` method will utilize the `attribute_exists()` parameter dynamically to ensure records are only "patched" and not created when updating items in your table.
...
[![Try it out!](https://img.shields.io/badge/electrodb-try_out_this_example_›-%23f9bd00?style=for-the-badge&logo=amazondynamodb&labelColor=1a212a)](https://electrodb.fun/?#code/PQKgBAsg9gJgpgGzARwK5wE4Es4GcA0YuccYGeqCALgUQBYCG5YA7llXWAGbZwB2MXGBDAAUKKwBbAA5QMVMAG8wAUT5V2AT0IBlTADcsAY1IBfbhiiSwAIkRwjVSzABGNgNzijUPrgVUGFwRSAF5bTShUDAB9AKC4aL4GSTgPcVAwABUGXABrITUNKk1hMW9ffxz8sDC+OBZVdS0AClEwJTb2sElYRAAuDq6u-iLNAZsAvNwbfE6h-UxcLB9xgEYZua7iDEMTccnchmlpG03TWaGGKicsF1QqPAHFTfaH5KeXruLpOHG-bD4AHMNkMhuQ0FhyDABk50J9zp8Dh9QV9ND8-jcgSCUWQ4BCoTCMHCUQiUdJLAArBxUZE476-Wz-LBYi448GoSFwaFgWFweGsoaoba0lH0jEA4EC0Hsznc3n8xHsYIi0FixmYyWfdoygk8olwKXtUmg+C4IzYaQaFaDOlohk2JlYhUovxXIUqobAYBgIWkBh8MBMDAMEpUKA8u2BgP8VCSLUR9FgADaNigPz4M1szIAtOSoIDyLhpoRU3xs3QoAgYJmbEYEFBiNWALqBoTlPyGrrwLgMSg02xp-inEmd2TMmge1GJmx8WMuTDYobGoYBQG4SevO1-OBUReg9hwSTr9US4eg5ddbySFLqY-PHEJ+0ILB+PdDA9HjdT+2SI5v0F5j88g4He8aCsKNoPiuW4nsywJgV0F4Pi4sBjJBUGbtOjrwRhRoIaY8YESOnx1g2XJfmqDoav+7T6Awz4wFcDLAEmAAM2YAJxNooAAspjZmxnHcQATPxglcYoonAJ2SFgOacBMTAACC--Lkz1gK58lMXJVhvQeYC5HAmhCMVIXSEwGj0UQcgKMjJRoJgIEIa9DYHp9j2UtSNYHL9hHOqCORLICdTcmpKLQ5WwSOMsviBgg9YNOjKDoLwQgMOaDZCJIfZYNIwRgCM7Akw+3iC9SfPjMzWCszeNC6XtcD7eMa5YKsV3Zmbqx5NmRv7bpV03VD3q-tIlVgBEURgAA4joACSYAABI5HQwAAApYyrPhIyjCGgw91tPQDL3w+TH3JjYvo6X9D6ye0eTO65bse17IV+4HABK-qAnAwA6HjseaPHOBg7Y1t5JDZPvQyKaulQQr03njNDC4ku5PWgJfsXRwexwpAV2ASQpGAMdl1ZmjL1AlkAEI8oEwTxvbptLMJltm8Jtv247QMcziCcn1gZ8QyncMyOnvdU1AZ27rnOL54DZ2D926n07q-buFNM7fRrKRRsv0AZEXPGcToxplBxEVgRYynhRAHFwJ0GymNmp0GaHfb64xvDkD3EiWwPEACs2YABsDDhJ7lOtScYY8jDIwEGeUwxl8GdRIUQAIA9jypnTDYMAfD8EMBgDAIRY5bwDFoVI-h7QbJHHTPIu+V59bHiTHfdo2dxgUn9Duf8KEYBoRsJkOgL4iAkCPGARgCAuDZjHsjGANkzxGj-tI9RLBRrkGaM0JQ-chTnCUHicws0LJ4maOE2gNgcx5gLHgaYxk1FgBsoCKAJl3BAA)
...
1. Using the `update` method to create new items when multiple mutations are present (In the future, this use case will be solved using the `upsert` method)
2. Migrating existing entities in place to use a new secondary index (best used with `patch`).
3. Updating attributes that appear in an index composite, when one ore more of the other composite attributes are readOnly.
...
#### Example
```typescript
entity.patch({ attr1: "value1", attr2: "value2" })
  .set({ attr4: "value4" })
  .go();
```
