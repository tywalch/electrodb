...
> When a transaction is canceled, due to conflict or other failure, ElectroDB will detail about the nature of the failure on each individual operation. By default, ElectroDB will return a reason for the failure, but if you provide the Execution Option `{ response: 'all_old' }` to the mutation `.commit()` function, ElectroDB will return the currently stored item (if it exists) on failure.

### Mutations
The mutations available on each injected entity are identical to the mutations available on all entities, with the addition of a transaction specific method `check`. Below are the mutations available on the injected entities and the corresponding DynamoDB parlance for each.

> The `check` method exists only within a transaction. It is similar to a `get` method in that you must provide all composite attribute's, but unlike `get` you can use the `where` clause to apply a condition expression.
...
--------------- | :--------------------: | -----------
failed          | `boolean`              | If the transaction failed to write, this value will be `true`
