...
> The example below demonstrates the shape of a write transaction. See the Unique Constraint example below for a runnable transaction.
...
The following examples use two [Entities](/en/modeling/entities), and a [Service](/en/modeling/services) to join them. This `Service` contains entities/information the top-secret British military intelligence organization: MI6. The `agent` entity models records for each MI6 agent and their personnel information, and the `constraint` entity is a utility available to all entities within the `MI6` service that can be used to enforce uniqueness for any property within a namespace. Services allow you to build namespaces with a single table, in this case a service called `mi6` joins the two entities together.
...
<LiveExample name="transact-write-unique-constraint" />
...
Below is an example of how you might use a `ClientRequestToken`. The `token` (a `ClientRequestToken` in DynamoDB parlance) should be unique for a given command. It can be helpful to use a deterministic value (like a composite string) to ensure it is always the same for a given command. Regardless of how many times `incrementAgentKills` is called, so long as the value for `token` remains the same, and you remain within DynamoDB's timing window, your operation will not duplicate your incrementation. Using this functionality can greatly simply your retry logic in the case of failure.

> The **Entities** and **Service** tabs show the setup this example runs against, and the generated parameters below are produced live by executing the example.

<LiveExample name="transact-write-idempotent-writes" />
