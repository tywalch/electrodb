...
> Unlike the `patch` method, `update` has a return type of [`Partial<EntityItem>`](/en/reference/typescript#entityitem-type). This is because an `update` is not considered a "safe" operation because can result in an item being accidentally created by DynamoDB. Accidental creations via `update` can result in partial items and therefore incorrect typing.
