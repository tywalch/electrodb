...
This maps to DynamoDB's [`ReturnValuesOnConditionCheckFailure`](https://docs.aws.amazon.com/amazondynamodb/latest/APIReference/API_PutItem.html#DDB-PutItem-request-ReturnValuesOnConditionCheckFailure) parameter and is supported on all write operations: `put`, `create`, `update`, `patch`, `delete`, `remove`, and `upsert`.

> **Note:** This feature requires AWS SDK v3, or AWS SDK v2 >= `2.1408.0`. Older v2 SDK versions do not include `ReturnValuesOnConditionCheckFailure` in their API model, so the parameter is silently stripped before reaching DynamoDB.
...
> When this option is _not_ provided, the response format is unchanged (`{ data: YourItem }`).
