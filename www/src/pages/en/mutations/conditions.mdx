...
By default, a failed condition expression throws. The `returnOnConditionCheckFailure` option turns the rejection into a return value.

Supported on `put`, `create`, `update`, `patch`, `delete`, `remove`, `upsert`. Accepted values:
...
- **`response`**: On success, the `data` field respects the `response` option (e.g. `"all_new"` returns the updated item). On rejection, `data` always contains the existing (old) item.
- **`data`**: Works with `"raw"` and `"includeKeys"` — the rejected item is returned in the requested format.
- **`originalErr`**: Does not affect condition check handling — failed conditions always resolve with `{ rejected: true }`. It only changes how other DynamoDB errors are thrown.
