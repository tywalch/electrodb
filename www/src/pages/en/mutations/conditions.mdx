...
animals
  ...
  .where(
    ({ isPregnant, lastEvaluation }, { lt, eq, and }) =>
      and(eq(isPregnant, true), lt(lastEvaluation, "2021-09-12")),
  )
  .go();
```
...
animals
  ...
  .where(
    (attr, op) =>
      op.and(op.eq(attr.dangerous, true), op.notExists(attr.lastFed)),
  )
  .go();
```
...
animals
  ...
  .where(
    (attr, op) =>
      op.or(op.eq(attr.dangerous, true), op.notExists(attr.lastFed)),
  )
...
| `and`         | `and(eq(a, b), eq(c, d))`        | `(#a = :a1 AND #c = :c1)`          |
| `or`          | `or(eq(a, b), eq(c, d))`         | `(#a = :a1 OR #c = :c1)`           |
...
const result = await StoreLocations
  .create({ cityId: "Atlanta1", storeId: "LatteLarrys", mallId: "EastPointe", buildingId: "BuildingA1", unitId: "B47", category: "food/coffee", leaseEndDate: "2020-03-22", rent: "4500.00" })
...
let stores = await MallStores.query
  ...
  .where(
    ({ rent, discount }, { between, eq, and }) =>
      and(between(rent, "2000.00", "5000.00"), eq(discount, "1000.00")),
  )
  .where(
    ({ category }, { eq }) => `
