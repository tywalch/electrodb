...
### Book
The "book" entity holds represents a physical book at the library because book details (like the the author or release date) do not change over time, we can use denormalization to remove the need for a single book entity. This allows us to treat this entity as both an authority on book information and an individual book.
...
### Get all books by last name and a partial first name.
...
```typescript
const { data, cursor } = await book.query.copies({ isbn: '9783453435773' }).go();
```
