...
const getResults = await docClient.send(new GetCommand({...}));
const queryResults = await docClient.send(new QueryCommand({...}));
const updateResults = await docClient.send(new UpdateCommand({...}));
const formattedGetResults = myEntity.parse(getResults);
const formattedQueryResults = myEntity.parse(queryResults);
```
