...
**What to do about it:**
Tables can be defined on the [Service Options](/en/modeling/service#options) object when you create an Entity or Service, or if that is not known at the time of creation, it can be supplied as a [Execution Options](/en/core-concepts/executing-queries) and supplied on each query individually. If can be supplied on both, in that case the Query Option will override the Service Option.

### Invalid Concurrency Option
**Code: 2004**

**Why this occurred:**
When performing a bulk operation ([Batch Get](/en/queries/batch-get), [Batch Delete Records](/en/mutations/batch-delete), [Batch Put Records](/en/mutations/batch-put)) you can pass a [Execution Options](/en/core-concepts/executing-queries) called `concurrent`, which impacts how many batch requests can occur at the same time. Your value should pass the test of both, `!isNaN(parseInt(value))` and `parseInt(value) > 0`.
...
**Why this occurred:**
When performing a query [Query](#building-queries) you can pass a [Execution Options](/en/core-concepts/executing-queries) called `pages`, which impacts how many DynamoDB pages a query should iterate through. Your value should pass the test of both, `!isNaN(parseInt(value))` and `parseInt(value) > 0`.
...
**Why this occurred:**
When performing a query [Query](#building-queries) you can pass a [Execution Options](/en/core-concepts/executing-queries) called `limit`, which impacts how many DynamoDB items a query should return. Your value should pass the test of both, `!isNaN(parseInt(value))` and `parseInt(value) > 0`.
