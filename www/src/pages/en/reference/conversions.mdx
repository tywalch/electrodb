...
// from Composite to Keys for a specific access pattern
thing.conversions.byAccessPattern.records.fromComposite.toKeys(item, {strict: 'all'});
// from Composite to Cursor for a specific access pattern
thing.conversions.byAccessPattern.records.fromComposite.toCursor(item, {strict: 'all'});
```
