...
> **Important:** Calling `.go()` does not immediately execute the query. The request to DynamoDB is deferred until the result is consumed — either by `await`, `.then()`, `.catch()`, `.finally()`, or `for await...of`. If you call `.go()` without consuming the result, the query will never execute.
...
For `query`, `scan`, and `collection` operations, the result of `.go()` can also be used as an `AsyncIterable` to iterate through pages of results one at a time. Read more in the [Pagination](/en/queries/pagination#async-iteration) section.
