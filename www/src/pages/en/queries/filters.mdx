...
animals.query
  ...
  .where(
    ({ isPregnant, offspring }, { exists, eq, or }) =>
      or(eq(isPregnant, true), exists(offspring)),
  )
  .go();
```
...
Operations return the strings that will be directly used in your FilterExpression. This means you always have fine-grain
control over any FilterExpression. This example joins the results of the `op.eq` operation with an `AND` clause, instead
of using the `op.and` function.
...
function getAnimals(options: GetAnimalOptions) {
  ...
  .where((attr, op) => {
    return keepers.map((name) => op.eq(attr.keeper, name)).join(' AND ');
  }).go();
}
