...
The example below uses a `Service` called `workforce` that joins together an `employee` entity and an `office` entity so that an employee's record and their office's record can be retrieved together, atomically.
...
<LiveExample name="transact-get-employee-and-office" />
