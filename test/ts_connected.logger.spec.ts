...
function createTestEntity(
  ...
  listeners?: ElectroEventListener[],
) {
  return new Entity(
    {
      model: {
...
type TestEntityCreateEntityItem = CreateEntityItem<
...
type TestListenerCallback = (options: {
  entity: TestEntityInstance;
  service: TestServiceInstance;
}) => Promise<TestListenerCallbackResponse>;
...
function applyEventComparisons(
  options: TestEventComparison,
  events: ElectroEvent[],
) {
  switch (options.comparison) {
    ...
    case "between":
      expect(events.length)
        .to.be.greaterThanOrEqual(options.calls[0])
        .and.lessThanOrEqual(options.calls[1]);
      break;
    case "in":
...
function equalCallCount(calls: number, events: ElectroEvent[]) {
  const queryEventsListenedTo = events.filter(
    (event) => event.type === "query",
  );
  const resultEventsListenedTo = events.filter(
    (event) => event.type === "results",
  );
...
describe("listener functions", () => {
  ...
  describe("listener service to entity propagation", () => {
    ...
    function createService(options: {
      logger?: (event: ElectroEvent) => void;
      listeners?: Array<(event: ElectroEvent) => void>;
    }) {
