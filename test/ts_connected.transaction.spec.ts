...
import { DynamoDBClient as V3Client, TransactGetItemsOutput } from '@aws-sdk/client-dynamodb';
import { expect } from "chai";
import { v4 as uuid } from "uuid";
import { Entity, Service, CreateEntityItem, EntityItem } from "../index";
        ...
        describe(`with ${type} client`, () => {
            describe('transactGet', () => {
                it(`should create parameters`, () => {
                    ...
                    const params = service.transaction.get(({team, task}) => [
                ...
                it(`should accept a request token on transaction requests`, async () => {
                    ...
                    await task.put({taskId, teamId, projectId, description, title}).go();

                    const result1 = await service.transaction.get(({team, task}) => [
                        team.get({teamId}).commit(),
                        task.get({taskId, teamId, projectId}).commit(),
                    ]).go({ token });

                    const result2 = await service.transaction.get(({team, task}) => [
                        team.get({teamId}).commit(),
                        task.get({taskId, teamId, projectId}).commit(),
                    ]).go({ token });
                ...
                it('should perform transaction operation', async () => {
                    ...
                    const getTeamAndTask = async (options: {
                        ...
                        return service.transaction.get(({task, team}) => [
                            task.get({taskId, teamId, projectId}).commit(),
                            team.get({teamId}).commit(),
                        ]).go()
                    }
            ...
            describe('transactWrite', () => {
                it(`should create parameters`, () => {
                    ...
                    const params = service.transaction.write(({task, team}) => [
                        task.put({taskId, title, teamId, projectId, description})
                            .where((attr, op) => `
                       ${op.notExists(attr.title)}
                   `)
                            .commit(),
                        task.create({taskId, title, teamId, projectId, description})
                            .where((attr, op) => `
                       ${op.notExists(attr.title)}
                   `)
                            .commit(),
                        task.upsert({taskId, title, teamId, projectId, description})
                            .where((attr, op) => `
                       ${op.notExists(attr.title)}
                   `)
                            .commit(),
                        task.update({taskId, teamId, projectId})
                            .set({ title })
                            .where((attr, op) => `
                       ${op.notExists(attr.title)}
                   `)
                            .commit(),
                        task.patch({taskId, teamId, projectId})
                            .set({ title })
                            .where((attr, op) => `
                       ${op.notExists(attr.title)}
                   `)
                            .commit(),
                        task.delete({taskId, teamId, projectId})
                            .where((attr, op) => `
                       ${op.notExists(attr.title)}
                   `)
                            .commit(),
                        task.remove({taskId, teamId, projectId})
                            .where((attr, op) => `
                       ${op.notExists(attr.title)}
                   `)
                            .commit(),
                        task.check({taskId, teamId, projectId})
                            .where((attr, op) => `
                       ${op.notExists(attr.title)}
                   `)
                            .commit(),

                        team.put({teamId, teamLead, teamName, department})
                            .where((attr, op) => `
                       ${op.notExists(attr.teamLead)}
                   `)
                            .commit(),
                        team.create({teamId, teamLead, teamName, department})
                            .where((attr, op) => `
                       ${op.notExists(attr.teamLead)}
                   `)
                            .commit(),
                        team.upsert({teamId, teamLead, teamName, department})
                            .where((attr, op) => `
                       ${op.notExists(attr.teamLead)}
                   `)
                            .commit(),
                        ...
                        TransactItems: [
                            ...
                            },
                            {
                                Put: {
                                    ...
                                    TableName: 'electro',
                                    ConditionExpression: 'attribute_not_exists(#pk) AND attribute_not_exists(#sk) AND attribute_not_exists(#title)',
                            ...
                            },
                            {
                                Update: {
                                    ...
                                    ExpressionAttributeValues: {
                                        ...
                                        ':description_u0': 'Create the most delicious burger ever made',
                                        ':taskId_u0': 'task0001',
                            ...
                            {
                                Update: {
                                    UpdateExpression: 'SET #title = :title_u0, #teamId = :teamId_u0, #projectId = :projectId_u0, #taskId = :taskId_u0, #__edb_e__ = :__edb_e___u0, #__edb_v__ = :__edb_v___u0',
                                    ExpressionAttributeNames: {
                                        '#title': 'title',
                                        '#teamId': 'teamId',
                                        '#projectId': 'projectId',
                            ...
                            },
                            {
                                Update: {
                                    ...
                                    ConditionExpression: 'attribute_exists(#pk) AND attribute_exists(#sk) AND attribute_not_exists(#title)'
                                },
                            },
                            {
                                Delete: {
                            ...
                            },
                            {
                                Put: {
                                    ...
                                    TableName: 'electro',
                                    ConditionExpression: 'attribute_not_exists(#pk) AND attribute_not_exists(#sk) AND attribute_not_exists(#teamLead)',
                                    ExpressionAttributeNames: { '#pk': 'pk', '#sk': 'sk', '#teamLead': 'teamLead' }
                                },
                            },
                            {
                                Update: {
                            ...
                            {
                                Update: {
                                    UpdateExpression: 'SET #teamLead = :teamLead_u0, #teamId = :teamId_u0, #__edb_e__ = :__edb_e___u0, #__edb_v__ = :__edb_v___u0',
                                    ExpressionAttributeNames: {
                                        '#teamLead': 'teamLead',
                                        '#teamId': 'teamId',
                                        '#__edb_e__': '__edb_e__',
                            ...
                            },
                            {
                                Update: {
                                    ...
                                    ConditionExpression: 'attribute_exists(#pk) AND attribute_exists(#sk) AND attribute_not_exists(#teamLead)'
                                },
                            },
                            {
                                Delete: {
                ...
                it(`should accept a request token on transaction requests`, async () => {
                    ...
                    const params = service.transaction.write(({task, team}) => [
                        task.put({taskId, teamId, projectId, description, title})
                    ...
                    const result1 = await service.transaction.write(({task, team}) => [
                        task.put({taskId, teamId, projectId, description, title})
                            .where((attr, op) => op.notExists(attr.title))
                            .commit(),
                    ]).go({ token });

                    const result2 = await service.transaction.write(({task, team}) => [
                        task.put({taskId, teamId, projectId, description, title})
                            .where((attr, op) => op.notExists(attr.title))
                            .commit(),
                    ]).go({ token });
                ...
                it('should perform transaction operation', async () => {
                    ...
                    const createNewAgent = async (newAgent: CreateEntityItem<typeof agent>) => {
                        return service.transaction.write(({agent, uniqueConstraint}) => [
                    ...
                    const updateAgentHandler = async (options: {id: string, handler: string}) => {
                        const {id, handler} = options;
                        return service.transaction.write(({agent}) => [
                            agent.update({id})
                                .set({handler})
                                .commit({response: 'all_old'}),
                            agent.check({id: handler})
