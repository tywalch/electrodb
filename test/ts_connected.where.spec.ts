...
describe("Where Clause Queries", () => {
    ...
    await Promise.all(
      animals.map((animal) => {
        ...
        if (animal === "Shark") {
          ...
          return WhereTests.put({ pen, row, animal, tags, codes })
            .go()
            .then((res) => res.data);
        }
      }),
  ...
  it("should filter 'field' with 'where'", async () => {
    ...
    const unknownFieldFiltered = await WhereTests.query
      .farm({ pen })
      .where(
        (_, { field, escape }) => `
    ...
    const knownFieldFiltered = await WhereTests.query
      .farm({ pen })
    ...
    expect(knownFieldFiltered.data.map((pen) => pen.animal)).to.have.members([
      "Cow",
    ]);
  });
  ...
  it("Should filter 'notContains' with 'where' on string set attribute", async () => {
    let animals = await WhereTests.query
      ...
      .where(
        ({ tags }, op) =>
          `${op.contains(tags, "hooves")} and ${op.notContains(
            tags,
            "edible",
          )}`,
      )
      ...
      .then((res) => res.data);
    expect(animals).to.be.an("array").and.have.length(1);
    expect(animals.map((pen) => pen.animal)).to.have.members(["Sheep"]);
  });
  ...
  it("Should filter 'notContains' with 'where' on numeric set attribute", async () => {
    let animals = await WhereTests.query
      ...
      .where(
        ({ codes }, op) =>
          `${op.contains(codes, 5)} and ${op.notContains(codes, 4)}`,
      )
      .go()
      .then((res) => res.data);
    expect(animals).to.be.an("array").and.have.length(3);
  ...
  it("Should allow for value operations to be used more than once", async () => {
    let animals = await WhereTests.query
      ...
      .then((res) => res.data);
    expect(animals).to.be.an("array").and.have.length(2);
    expect(animals.map((pen) => pen.animal)).to.have.members(["Pig", "Shark"]);
  });
  ...
  it("should properly handle nested properties being used more than once", () => {
    ...
    const params = tasks.query
      ...
      .where(({ example: { from, to } }, { lte, gte, notExists }) => {
        return `${lte(from, epoch)} AND (${gte(to, epoch)} OR ${notExists(
          to,
        )})`;
      })
      .params();
  ...
  it("scan should start a new chain with each use", () => {
    ...
    const where1 = MyEntity.scan.where(({ complete }, { eq }) =>
      eq(complete, false),
    );
    expect(where1.params()).to.deep.equal({
      ...
      FilterExpression:
        "begins_with(#pk, :pk) AND begins_with(#sk, :sk) AND (#complete = :complete0) AND #__edb_e__ = :__edb_e__0 AND #__edb_v__ = :__edb_v__0",
    });

    const where2 = MyEntity.scan.where(({ complete }, { eq }) =>
      eq(complete, true),
    );
    ...
    const where3 = MyEntity.scan.where(({ complete }, { eq }) =>
      eq(complete, true),
    );
  ...
  it("should upsert with a condition", () => {
    ...
    const params = actors
      .upsert({
        productionHouse: "productionHouse1",
        studio: "studio1",
        project: "project2",
        actor: "actor1",
      })
    ...
    expect(params).to.deep.equal({
      TableName: "electro",
      UpdateExpression:
        "SET #__edb_e__ = :__edb_e___u0, #__edb_v__ = :__edb_v___u0, #studio = :studio_u0, #productionHouse = :productionHouse_u0, #project = :project_u0, #actor = :actor_u0, #gsi2pk = :gsi2pk_u0, #gsi2sk = :gsi2sk_u0",
      ExpressionAttributeNames: {
        "#project": "project",
        "#__edb_e__": "__edb_e__",
        "#__edb_v__": "__edb_v__",
  ...
  it("should apply a size filter on query", async () => {
    ...
    const { data } = await entity.query
      ...
      .where(
        ({ prop }, { size, escape, eq }) => `
                ${size(prop)} >= ${escape(minSize)} OR ${eq(
                  prop,
                  escape(exception),
                )} OR ${size(prop)} <= ${escape(maxSize)}
            `,
      )
      .go();
    ...
    expect(data.sort((a, z) => a.name.localeCompare(z.name))).to.deep.equal(
      items
        .sort((a, z) => a.name.localeCompare(z.name))
        .filter((item) => {
          return (
            (item.prop && item.prop.length >= minSize) ||
    ...
    expect(getParams(log)).to.deep.equal({
      ...
      FilterExpression:
        "size(#prop) >= :30 OR #prop = :a0 OR size(#prop) <= :50",
    });
  });
  ...
  it("should apply an attribute type filter on query", async () => {
    ...
    const expected = items
      .sort((a, z) => a.name.localeCompare(z.name))
      .filter(
        (item) => Array.isArray(item.prop) || typeof item.prop === "boolean",
      );
    ...
    expect(getParams(log)).to.deep.equal({
      ...
      FilterExpression:
        "attribute_type(#prop, :prop0) OR attribute_type(#prop, :prop1)",
    });
    ...
    it('should accept number literals via "escape"', async () => {
      ...
      const { data } = await entity.query
        ...
        .where(
          ({ prop }, { escape, eq, name }) => `
                    ${name(prop)} >= ${escape(minSize)} OR ${eq(
                      prop,
                      escape(exception),
                    )} OR ${name(prop)} <= ${escape(maxSize)}
                `,
        )
        .go();
