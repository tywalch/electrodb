...
describe("Entity", () => {
  ...
  describe("Simple crud", () => {
    ...
    it("Should not collide with other keys", async () => {
      ...
      let secondStoreBeforeUpdate = await MallStores.get(secondStore)
        .go()
        .then((res) => res.data);
      let newRent = "5000.00";
    ...
    it("providing upsert and create with the same data should result in the same item created", async () => {
      const updatedAt = Date.now();
      ...
      expect({ ...item1.data, accountNumber: null }).to.deep.equal({
        ...item2.data,
        accountNumber: null,
      });
    ...
    it("providing transact upsert and create with the same data should result in the same item created", async () => {
      const updatedAt = Date.now();
      function createThing() {
        let createdAt = 1;
        return new Entity(
      ...
      const upsertParams = thingService.transaction.write(({ thing1 }) => {
        return [
          thing1.upsert({
            transactionId,
            accountNumber: accountNumber1,
    ...
    it("Should not create a overwrite existing record", async () => {
      ...
      let recordOne = await MallStores.create(record)
        .go()
        .then((res) => res.data);
      ...
      let recordTwo = null;
      try {
        recordTwo = await MallStores.create(record)
          .go()
          .then((res) => res.data);
      } catch (err: any) {
  ...
  describe("Simple crud without sort key", () => {
    ...
    it("Should not collide with other keys", async () => {
      ...
      let secondStoreAfterUpdate = await MallStores.get(secondStore)
        .go()
        .then((res) => res.data);
  ...
  describe("Tailing labels", async () => {
    it("Should include tailing label if chained query methods are not used", async () => {
      ...
      await sleep(500);
      let beginsWithIsle = await labeler.query
        .locations({ section })
        .begins({ isle: "A2" })
        .go()
        .then((res) => res.data);
      let specificIsle = await labeler.query
        .locations({ section, isle: "A2" })
        .go()
        .then((res) => res.data);
  ...
  describe("Delete records", () => {
    it("Should create then delete a record", async () => {
      ...
      await sleep(150);
      let recordNoLongerExists = await record
        .get({ prop1, prop2 })
        .go()
        .then((res) => res.data);
    ...
    it("Should throw when trying to remove a record that does not exist", async () => {
      ...
      let prop1 = uuid();
      let prop2 = uuid();
      let recordNotExists = await record
        .get({ prop1, prop2 })
        .go()
        .then((res) => res.data);
  ...
  describe("Filters", () => {
    ...
    it("Should allow for multiple filters", async () => {
      ...
      let found = await db.query
        ...
        .where(
          ({ color, id }, { notContains, contains }) => `
                    ${notContains(color, "green")} OR ${contains(
                      id,
                      "weird_value",
                    )}
                `,
  ...
  describe("Updating Records", () => {
    ...
    it("Should update only completed GSI pk keys", async () => {
      ...
      await sleep(100);
      let beforeUpdateQueryParams = Dummy.query
        .index3({
          prop5: record.prop5,
          prop6: record.prop6,
          prop7: record.prop7,
      ...
      expect(beforeUpdate)
        .to.be.an("array")
        .with.lengthOf(1)
        .and.to.deep.equal([record]);
      await Dummy.update({ prop1: record.prop1, prop2: record.prop2 })
        .set({ prop9, prop5 })
        .go()
        .then((res) => res.data);
      await sleep(100);
      let afterUpdateQueryParams = Dummy.query
        .index3({
          prop5,
          prop6: record.prop6,
          prop7: record.prop7,
      ...
      expect(afterUpdate)
        .to.be.an("array")
        .with.lengthOf(1)
        .and.to.deep.equal([{ ...record, prop9, prop5 }]);
    });
    ...
    it("Should update only completed GSI sk keys", async () => {
      ...
      await Dummy.put(record)
        .go()
        .then((res) => res.data);
      await sleep(100);
      let beforeUpdateQueryParams = Dummy.query
        .index2({ prop3: record.prop3, prop4: record.prop4 })
        .params();
      expect(beforeUpdateQueryParams).to.deep.equal({
        KeyConditionExpression: "#pk = :pk and #sk1 = :sk1",
        TableName: "electro",
      ...
      await sleep(100);
      let afterUpdateQueryParams = Dummy.query
        .index2({ prop3: record.prop3, prop4 })
        .params();
      expect(afterUpdateQueryParams).to.deep.equal({
        KeyConditionExpression: "#pk = :pk and #sk1 = :sk1",
        TableName: "electro",
      ...
      expect(afterUpdate)
        .to.be.an("array")
        .with.lengthOf(1)
        .and.to.deep.equal([{ ...record, prop9, prop4 }]);
    });
  ...
  describe("ReturnValues", () => {
    ...
    for (const method of ["delete", "remove"] as const) {
      it(`should return all old values when using ${method}`, async () => {
        ...
        const deleteParams1: any = db[method]({ id, sub }).params({
          response: "all_old",
        });
  ...
  describe("Item parsing", () => {
    ...
    it("should parse the response from an item", async () => {
      ...
      const parsedTrimmed = entity.parse(itemFromDocClient, {
    ...
    it("should parse the response from an query that lacks identifiers", async () => {
      ...
      const parsed = entity.parse(itemFromDocClient);
      expect(parsed.data).to.deep.equal([{ prop1, prop2, prop3 }]);
      const parseTrimmed = entity.parse(itemFromDocClient, {
        attributes: ["prop1", "prop3"],
      });
    ...
    it("should parse the response from a delete", async () => {
      ...
      const parsed = entity.parse(results);
      expect(parsed.data).to.deep.equal({ prop1, prop2, prop3 });
      const parseTrimmed = entity.parse(results, {
        attributes: ["prop1", "prop3"],
      });
      expect(parseTrimmed.data).to.deep.equal({ prop1, prop3 });
    });
    ...
    it("should parse the response from a scan", async () => {
      ...
      expect(parsed.data).to.deep.equal([{ prop1, prop2, prop3 }]);
      const parseTrimmed = entity.parse(scanResponse, {
        attributes: ["prop1", "prop3"],
      });
      expect(parseTrimmed.data).to.deep.equal([{ prop1, prop3 }]);
    }).timeout(10000);
  ...
  describe("Key fields that match Attribute fields", () => {
    ...
    it("attribute keys can have prefixes and postfixes when defined with a prefix and postfix", async () => {
      ...
      const newUpdateParameters = entity
        .update(newRecord)
        .set({ createdAt })
        .add({ incrementId: 1 })
        .params();
      ...
      expect(newUpdateParameters).to.deep.equal({
        UpdateExpression:
          "SET #createdAt = :createdAt_u0, #__edb_e__ = :__edb_e___u0, #__edb_v__ = :__edb_v___u0 ADD #incrementId :incrementId_u0",
        ExpressionAttributeNames: {
          "#createdAt": "createdAt",
        ...
        ExpressionAttributeValues: {
          ":createdAt_u0": `pre#${createdAt}#post`,
          ":incrementId_u0": 1,
          ":__edb_e___u0": "accounts",
          ":__edb_v___u0": "1",
        },
        TableName: "electro_keynamesattributenames",
        Key: {
      ...
      const query1 = await entity.query
      ...
      const query2 = await entity.query
        .onboarded({ type: newRecord.type })
        .where(({ accountId }, { eq }) =>
          eq(accountId, newRecord.accountId.toUpperCase()),
        )
        .go()
        .then((res) => res.data);
    ...
    it("Should throw when trying to add a prefix or postfix to a number attribute being used as an index field", () => {
      expect(() => {
        ...
        'definition for "sk" field on index "(Primary Index)". Index templates may only have prefix or postfix values on "string" or "enum" type attributes. The sk field "incrementId" is type "number", and therefore cannot be used with prefixes or postfixes. Please either remove the prefixed or postfixed values from the template or change the field name of the attribute. - For more detail on this error reference: https://electrodb.dev/en/reference/errors/#invalid-index-with-attribute-name',
      );
    });
  });
});
...
describe("pagination order", () => {
  it("should return batchGet results in the order their keys were provided regardless of the order returned by DynamoDB", async () => {
        ...
        return {
          promise: async (): Promise<any> => {
            ...
            const outOfOrderItem = after.find((item, i) => {
              return item.pk !== before[i].pk && item.sk !== before[i].sk;
            });
    ...
    const unOrderedRecords = await entity
      .get(batch)
      .go()
      .then((res) => res.unprocessed);
  ...
  it("should return batchGet results in the order their keys were provided with null values in the place of unprocessed keys", async () => {
        ...
        return {
          promise: async (): Promise<any> => {
            ...
            for (let i = 0; i < countToMakeUnprocessed; i++) {
              ...
              results.UnprocessedKeys[table] = results.UnprocessedKeys?.[
                table
              ] ?? {
                Keys: [],
              };
    ...
    const totalUnprocessed =
      Math.round(batchCount / 100) * countToMakeUnprocessed;
    const [unOrderedRecords, unOrderedUnprocessed] = await entity
      .get(batch)
      .go()
    ...
    expect(unOrderedUnprocessed)
      .to.be.an("array")
      .with.length(totalUnprocessed);
    // todo: internals now need to care about .data
    const [orderedRecords, orderedUnprocessed] = await entity
      .get(batch)
    ...
    for (let i = 0; i < orderedRecords.length; i++) {
      ...
      const fromBatch = batch[i];
      if (fromDb === null) {
        const fromOrderedUnprocessed = orderedUnprocessed.find(
          (unprocessed) => {
            return (
              unprocessed.prop1 === fromBatch.prop1 &&
              unprocessed.prop2 === fromBatch.prop2
            );
          },
        );
...
describe("attributes query option", () => {
  ...
  it("should not add entity identifiers", async () => {
    ...
    expect(queryParams.ProjectionExpression).to.equal(
      "#pk, #sk1, #attr2, #prop9, #attr5, #attr10",
    );

    // raw
    const getRaw = await entityWithSK
      .get({
        attr1: item.attr1,
        attr2: item.attr2,
      })
    ...
    const queryRawPage = await entityWithSK.query
      .myIndex({
        attr1: item.attr1,
        attr2: item.attr2,
      })
    ...
    // pagerRaw
    const queryRawPager = await entityWithSK.query
      .myIndex({
        attr1: item.attr1,
        attr2: item.attr2,
      })
    ...
    const getIgnoreOwnership = await entityWithSK
      .get({
        attr1: item.attr1,
        attr2: item.attr2,
      })
    ...
    const queryIgnoreOwnershipGo = await entityWithSK.query
      .myIndex({
        attr1: item.attr1,
        attr2: item.attr2,
      })
    ...
    const queryIgnoreOwnershipPage = await entityWithSK.query
      .myIndex({
        attr1: item.attr1,
        attr2: item.attr2,
      })
  ...
  it("should return all values if attributes is empty array", async () => {
    ...
    // params
    const getParams = await entityWithSK
      .get({
        attr1: item.attr1,
        attr2: item.attr2,
      })
  ...
  it("should include index composite attributes on automatically but not on the response", async () => {
    ...
    await entityWithSK.put(item).go();
    let params: any;
    const results = await entityWithSK.query
      .myIndex2({
        attr5: item.attr5,
        attr4: item.attr4,
        attr6: item.attr6!,
  ...
  it("should evaluate queries as complete when all sort keys are provided to the access pattern method", async () => {
    ...
    expect(partial.KeyConditionExpression).to.equal(
      "#pk = :pk and begins_with(#sk1, :sk1)",
    );

    const full = entity.query
      .record({
        prop1: "prop1",
        prop2: "prop2",
        prop3: "prop3",
    ...
    const tests = await entity.query
      .record({ prop1, prop2, prop3: "test" })
      .go()
      .then((res) => res.data);
    expect(tests).to.deep.equal([{ prop1, prop2, prop3: "test" }]);
    const testish = await entity.query
      .record({ prop1, prop2 })
      .begins({ prop3: "test" })
      .go()
      .then((res) => res.data);
...
describe("attribute padding", () => {
  it("should perform crud operations with padded number attribute", async () => {
    ...
    const created = await entity
      ...
      .go();
    expect(created.data).to.deep.equal({
      prop1,
      prop2,
      prop3,
      prop4: initialProp4,
    ...
    expect(getFromInitial.data).to.deep.equal({
      prop1,
      prop2,
      prop3,
      prop4: initialProp4,
    ...
    const queryFromInitialProp1 = await entity.query
      .record({ prop1, prop2 })
      .go();
    expect(queryFromInitialProp1.data[0]).to.deep.equal({
      prop1,
      prop2,
      prop3,
      prop4: initialProp4,
    ...
    const queryFromInitialProp4 = await entity.query
      .second({ prop3, prop4: initialProp4 })
      .go();
    expect(queryFromInitialProp4.data[0]).to.deep.equal({
      prop1,
      prop2,
      prop3,
      prop4: initialProp4,
    ...
    const nextProp4 = 25;
    const updatedProp4 = await entity
      .update({ prop1, prop2 })
      .set({ prop4: nextProp4 })
      .go({ response: "all_new" });
    expect(updatedProp4.data).to.deep.equal({
      prop1,
      prop2,
      prop3,
      prop4: nextProp4,
    ...
    const queryFromNextProp4 = await entity.query
      .second({ prop3, prop4: nextProp4 })
      .go();
    expect(queryFromNextProp4.data[0]).to.deep.equal({
      prop1,
      prop2,
      prop3,
      prop4: nextProp4,
  ...
  it("should perform crud operations with padded string attribute", async () => {
    ...
    expect(getFromKnown.data).to.be.deep.equal({
      prop1,
      prop2,
      prop3,
      prop4: initialProp4,
    ...
    const queryFromKnownProp1 = await entity.query
      .record({ prop1: knownProp1, prop2 })
      .go();
    expect(queryFromKnownProp1.data[0]).to.deep.equal({
      prop1,
      prop2,
      prop3,
      prop4: initialProp4,
    ...
    const queryFromKnownProp4 = await entity.query
      .second({ prop3, prop4: knownProp4 })
      .go();
    expect(queryFromKnownProp4.data[0]).to.deep.equal({
      prop1,
      prop2,
      prop3,
      prop4: initialProp4,
    ...
    const queryFromKnownNextProp4 = await entity.query
      .second({ prop3, prop4: knownNextProp4 })
      .go();
    expect(queryFromKnownNextProp4.data[0]).to.deep.equal({
      prop1,
      prop2,
      prop3,
      prop4: nextProp4,
...
describe("upsert", () => {
  ...
  it("should create an item if one did not exist prior", async () => {
    ...
    const initialUpsert = await tasks
      .upsert({
        project,
        complete,
        task,
  ...
  it("should update an existing item", async () => {
    ...
    const upsertedOverExisting = await tasks
      .upsert({
        task,
        project,
        team,
...
describe("batch operations", () => {
  it('should return batchGet records when "table" is provided as query option', async () => {
    ...
    expect(
      batchGet.data.sort((a, z) => a.segmentId.localeCompare(z.segmentId)),
    ).to.deep.equal(
      items.sort((a, z) => a.segmentId.localeCompare(z.segmentId)),
    );
  });
});
...
describe("enum set", () => {
  ...
  it("should perform full crud on enum set", async () => {
    ...
    expect(fourth.data).to.deep.equal({
      id,
      tags: ["SCIENCE"],
      optional: "hi",
    });
  ...
  describe("ignore ownership", () => {
    ...
    it("should get results", async () => {
      ...
      const withElectroAndWithoutOwnership = await BlogEntry.get(
        withElectro,
      ).go({ ignoreOwnership: true });

      const withoutElectroAndOwnership =
        await BlogEntry.get(withoutElectro).go();
      const withoutElectroAndWithoutOwnership = await BlogEntry.get(
        withoutElectro,
      ).go({ ignoreOwnership: true });
      ...
      expect(withoutElectroAndWithoutOwnership.data).to.deep.equal(
        withoutElectro,
      );
    });
    ...
    it("should batch get results", async () => {
      ...
      const withElectroAndOwnership = await BlogEntry.get([
        withElectro,
        withoutElectro,
      ]).go({ preserveBatchOrder: true });
      const withElectroAndWithoutOwnership = await BlogEntry.get([
      ...
      expect(withElectroAndWithoutOwnership.data).to.deep.equal([
        withElectro,
        withoutElectro,
      ]);
    });
    ...
    it("should query results", async () => {
      ...
      const withElectroAndOwnership = await BlogEntry.query
        .record(withElectro)
        .go();
      const withElectroAndWithoutOwnership = await BlogEntry.query
        .record(withElectro)
        .go({ ignoreOwnership: true });
      const withElectroAndOwnershipGSI = await BlogEntry.query
        .userEntries(withElectro)
        .go();
      const withElectroAndWithoutOwnershipGSI = await BlogEntry.query
        .userEntries(withElectro)
        .go({ ignoreOwnership: true });

      const withoutElectroAndOwnership = await BlogEntry.query
        .record(withoutElectro)
        .go();
      const withoutElectroAndWithoutOwnership = await BlogEntry.query
        .record(withoutElectro)
        .go({ ignoreOwnership: true });
      const withoutElectroAndOwnershipGSI = await BlogEntry.query
        .userEntries(withoutElectro)
        .go();

      const withoutElectroAndWithoutOwnershipGSI = await BlogEntry.query
        .userEntries(withoutElectro)
        .go({
          ignoreOwnership: true,
        });

      const partialQueryWithOwnership = await BlogEntry.query
        .userEntries({ userId: withElectro.userId })
        .go();
      const partialQueryWithoutOwnership = await BlogEntry.query
        .userEntries({ userId: withElectro.userId })
        .go({ ignoreOwnership: true });
      ...
      expect(withElectroAndWithoutOwnershipGSI.data).to.deep.equal([
        withElectro,
      ]);
      ...
      expect(withoutElectroAndWithoutOwnershipGSI.data).to.deep.equal([
        withoutElectro,
      ]);
      ...
      expect(
        partialQueryWithoutOwnership.data.sort((a, z) => {
          return a.id.localeCompare(z.id);
        }),
      ).to.deep.equal(
        [withoutElectro, withElectro].sort((a, z) => {
          return a.id.localeCompare(z.id);
        }),
      );
    });
...
describe("terminal methods", () => {
  it("should allow repeated calls to a terminal method without breaking side effects", async () => {
    ...
    const testOperation = async (
      name: string,
      operation: TestOperationOptions["operation"],
    ...
    await testOperation(
      "update",
      entity.update(createItem()).set({ name: "update" }),
    );
...
describe("scanning an index", () => {
  const primaryAccessPattern = "primary";
        ...
        byAdults: {
          ...
          condition(attrs) {
            if (typeof attrs.age !== "number") {
              throw new Error("age is required to maintain index integrity");
            }
            return attrs.age >= 18;
          },
          pk: {
  ...
  let kid: EntityItem<typeof entity>;
  let adult: EntityItem<typeof entity>;
  ...
  it("should include the index name in the params", async () => {
    const params = entity.scan.byAdults.params();
    expect(params.IndexName).to.equal("gsi1pk-gsi1sk-index");
    expect(params.ExpressionAttributeValues[":gsi1pk"]).to.equal("$tests");
    expect(params.ExpressionAttributeValues[":gsi1sk"]).to.equal(
      "$scan-index_1#age_",
    );
    expect(params.FilterExpression).to.contain(
      "begins_with(#gsi1pk, :gsi1pk) AND begins_with(#gsi1sk, :gsi1sk)",
    );

    const { data: onlyAdults } = await entity.scan.byAdults.go({
      pages: "all",
    });
    expect(onlyAdults.find((i) => i.id === adult.id)).to.exist;
    expect(onlyAdults.find((i) => i.id === kid.id)).to.not.exist;

    const { data: allPeople } = await entity.scan.go({ pages: "all" });
    expect(allPeople.find((i) => i.id === kid.id)).to.exist;
    expect(allPeople.find((i) => i.id === adult.id)).to.exist;
  });

  it("should allow using primary index", async () => {
    const params = entity.scan[primaryAccessPattern].params();
    expect(params.IndexName).not.to.exist;

    const { data: allPeople } = await entity.scan[primaryAccessPattern].go({
      pages: "all",
    });
    expect(allPeople.find((i) => i.id === kid.id)).to.exist;
    expect(allPeople.find((i) => i.id === adult.id)).to.exist;
...
describe("index projection", () => {
  ...
  const entity = createMainEntity(1);
  const entityV2 = createMainEntity(2);
  ...
  before(async () => {
    ...
    // we intentionally create a v2 item to make sure that ElectroDB
    // doesn't return this item when querying a collection with only v1
    // and when querying v1 indexes with limited projected attributes
    await entityV2.create(item).go();
    ...
    expect(resp).to.deep.equal([
      {
        ...
        include3: item.include3,
        exclude1: item.exclude1,
        exclude2: item.exclude2,
        exclude3: item.exclude3,
      },
    ]);
  });
  ...
  it("queries index with keys only and ignoreOwnership=false", async () => {
    const { data: resp } = await entity.query
      .keysOnly({ id: item.id })
      .go({ ignoreOwnership: false });
    // we return an empty array because ElectroDB can't verify entity ownership
    // for a keys only index
    expect(resp).to.deep.equal([]);
  });

  it("scans index with keys only and ignoreOwnership=false", async () => {
  ...
  it("automatically sets ignoreOwnership=true for INCLUDE index", async () => {
    const params = entity.scan.includeIndex.params({
      table: "electro_projectionincludewithoutedb",
      pages: "all",
    });
  ...
  it("automatically sets ignoreOwnership=true for KEYS_ONLY index", async () => {
    const keysOnlyParams = entity.scan.keysOnly.params();
    expect(keysOnlyParams.FilterExpression).to.not.include("__edb_e__");
    expect(keysOnlyParams.FilterExpression).to.not.include("__edb_v__");
  ...
  it("does not automatically set ignoreOwnership=true for ALL projection index", async () => {
    const allProjectionParams = entity.scan.thirdIndex.params();
  ...
  it("automatically sets ignoreOwnership=true for a collection with INCLUDE index", async () => {
    const collectionParams = service.collections
      .includeIndexCollection({ id: item.id })
      .params();
    expect(collectionParams.FilterExpression).to.be.undefined;
    expect(collectionParams.FilterExpression).to.be.undefined;

    const { data: collectionResp } = await service.collections
      .includeIndexCollection({ id: item.id })
      .go({
        pages: "all",
      });
    expect(collectionResp).to.deep.equal({
      entity: [
        {
          include1: item.include1,
          include2: item.include2,
  ...
  it("does not automatically set ignoreOwnership=true for a collection with ALL projection index", async () => {
    const allProjectionCollectionParams = service.collections
      .primaryCollection({ id: item.id })
      .params();
    expect(allProjectionCollectionParams.FilterExpression).to.include(
      "__edb_e__",
    );
    expect(allProjectionCollectionParams.FilterExpression).to.include(
      "__edb_v__",
    ...
    const { data: allProjectionCollectionResp } = await service.collections
      .primaryCollection({ id: item.id })
      .go({
        pages: "all",
      });
    expect(allProjectionCollectionResp).to.deep.equal({
      entity: [item],
      entity2: [item2],
    });
  });
