...
import { entityWithReadOnlyAttributes, entityWithComplexShapes, serivce } from "./upsert-entities.test-d";
...
function transactionUpsertTests() {
    const prop1 = "prop1";
    const prop2 = 5;
    const prop3 = { val1: "val1" };
    const prop4 = [
...
// upsert allows all values to be passed in
    serivce.transaction.write(({ entityWithComplexShapes }) => {
        return [
            entityWithComplexShapes.upsert({
                prop1,
                prop2,
...
// go is not available if minimum required attributes are supplied across different types of mutations is not met
        expectType<
            ...
            | "Required: requiredProp3"
            | "Required: requiredProp4"
        >(entityWithComplexShapes.upsert({}).commit);

        return [];
    });
// do not allow add, subtract, and append on readOnly attributes
    function readOnlyTests() {
        serivce.transaction.write(({ entityWithReadOnlyAttributes }) => {
            const prop1 = "value1";
            const prop2 = "value2";
            const prop3 = "value3";
            const prop3ReadOnly = prop3;
