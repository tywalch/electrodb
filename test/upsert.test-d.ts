...
expectType<'Missing required attributes to perform upsert' | 'Required: requiredProp5'>(entityWithComplexShapes.upsert({
    // table index keys
    prop1,
    prop2,
...
expectType<'Missing required attributes to perform upsert' | 'Required: prop1'>(entityWithComplexShapes.upsert({
    // table index keys
...
expectType<'Missing required attributes to perform upsert' | 'Required: prop1' | 'Required: requiredProp5'>(entityWithComplexShapes.upsert({
    // table index keys
    // implicit required attribute missing
    // prop1,
    prop2,
...
expectType<'Missing required attributes to perform upsert' | 'Required: requiredProp1' | 'Required: requiredProp5'>(entityWithComplexShapes
    .upsert({ prop2 })
    .set({ prop1 })
    .add({ prop5 })
    // .set({ requiredProp1 })
...
// go is not available if minimum required attributes are supplied across different types of mutations is not met
expectType<"Missing required attributes to perform upsert" | "Required: requiredProp5" | "Required: prop1" | "Required: requiredProp1" | "Required: prop2" | "Required: requiredProp2" | "Required: requiredProp3" | "Required: requiredProp4">(entityWithComplexShapes
    .upsert({})
    .go);
...
function readOnlyTests() {
    ...
    entityWithReadOnlyAttributes.upsert({
        prop1,
        prop2,
        prop3,
        prop3ReadOnly,
    ...
    entityWithReadOnlyAttributes.upsert({}).set({
        prop1,
        prop2,
        prop3,
        prop3ReadOnly,
