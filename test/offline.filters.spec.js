...
const { FilterFactory } = require("../src/filters");
const { WhereOperations } = require("../src/operations");
const { ChainState } = require("../src/clauses");
const { ExpressionTypes } = require("../src/types");
...
describe("Filter", () => {
	...
	describe("Clause Building", () => {
		...
		it("Should build a clause", () => {
			...
			expect(state.expressions.getNames()).to.deep.equal({
				"#rent": "rent",
				"#mall": "mallId",
				"#leaseEnd": "leaseEnd",
			});
			expect(state.expressions.getValues()).to.deep.equal({
				":rent1": "20.00",
				":mall1": "EastPointe",
				":leaseEnd1": "20200101",
				":leaseEnd2": "20200401",
			});
			expect(state.expressions.getExpression(ExpressionTypes.FilterExpression)).to.equal("(#rent >= :rent1 AND #mall = :mall1) OR (#leaseEnd between :leaseEnd1 and :leaseEnd2)")
		});
		it("Shouldnt validate the attributes passed when not strict", () => {
			...
			expect(state.expressions.getNames()).to.deep.equal({ '#category': 'category' });
			expect(state.expressions.getValues()).to.deep.equal({ ':category1': 'food' });
			expect(state.expressions.getExpression(ExpressionTypes.FilterExpression)).to.equal('contains(#category, :category1)');
		});
