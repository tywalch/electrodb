...
describe("Service Offline", () => {
  describe("TypeScript oriented constructor", () => {
    ...
    it("Should take a map of Models", () => {
      ...
      let service = new Service(entityMap);
      expect(service.entities.modelOne.model.entity).to.equal(
        modelOne.model.entity,
      );
      expect(service.entities.modelTwo.model.entity).to.equal(
        modelTwo.model.entity,
      );
      expect(service.entities.modelThree.model.entity).to.equal(
        modelThree.model.entity,
      );
    });
    describe("Should apply service configuration options onto entities", () => {
      ...
      it("Should apply a table name to each entity when not set on the entity itself", () => {
        ...
        let query = service.collections.morerecords({ attr1: "blah" }).params();
      ...
      it("Allow a table name to only be supplied via query options", () => {
        ...
        expect(query.TableName).to.equal(table);
        expect(() =>
          service.collections.morerecords({ attr1: "blah" }).params(),
        ).to.throw(
          "Table name not defined. Table names must be either defined on the model, instance configuration, or as a query option. - For more detail on this error reference: https://electrodb.dev/en/reference/errors/#missing-table",
        );
      });
  ...
  it("Should not allow a service to be created without a name", () => {
    expect(() => new Service()).to.throw(
      `Invalid service name: "". Service name must have length greater than zero - For more detail on this error reference: https://electrodb.dev/en/reference/errors/#join`,
    );
  });
  it("Should not allow a join to be performed on an object other than an Entity or Model", () => {
    let service = new Service("MyService");
    expect(() => service.join({ model: { entity: "beep-boop" } })).to.throw(
      `Invalid instance: Valid instances to join include Models and Entity instances. - For more detail on this error reference: https://electrodb.dev/en/reference/errors/#join`,
    );
  });
  it("Should not allow a join to be performed on an empty object", () => {
    let service = new Service("MyService");
    expect(() => service.join({})).to.throw(
      `Invalid instance: Valid instances to join include Models and Entity instances. - For more detail on this error reference: https://electrodb.dev/en/reference/errors/#join`,
    );
  });
  ...
  it("Should not allow joining a model with a different service name", () => {
    ...
    let service = new Service("MyService", { table: "MyTable" });
    let entity = new Entity(schema, { table: "MyTable" });
    expect(() => service.join(entity)).to.throw(
      "Service name defined on joined instance, MyOtherService, does not match the name of this Service: MyService. Verify or update the service name on the Entity/Model to match the name defined on this service.",
    );
  });
  ...
  it("Should require that all partition keys should be defined with the same casing", () => {
    ...
    expect(() => database.join(entityTwo)).to.throw(
  ...
  it("Should require matching PK values for entities associated with a common collection", () => {
    ...
    database.join(entityOne);
    expect(() => database.join(entityTwo)).to.throw(
      'Partition Key composite attributes provided ["prop1"] for index "(Primary Index)" do not match established composite attributes ["prop1", "prop7"] on established index "(Primary Index)" - For more detail on this error reference: https://electrodb.dev/en/reference/errors/#join',
    );
  });
  it("Should require all PK values", () => {
    ...
    database.join(entityOne).join(entityTwo);

    expect(() =>
      database.collections.collectionA({ prop1: "abc" }).params(),
    ).to.throw(
      'Incomplete or invalid key composite attributes supplied. Missing properties: "prop7"',
    );
    expect(
      database.collections
        .collectionA({ prop1: "abc", prop7: "def", prop2: "hij" })
        .params(),
    ).to.deep.equal({
...
describe("Misconfiguration exceptions", () => {
  it("Should should not allow joined entities to have the same collection name across different indexes", () => {
    ...
    database.join(entityOne);
    expect(() => database.join(entityTwo)).to.throw(
      `Collection defined on provided index "gis1" does not match collection established index "gis2". Collections must be defined on the same index across all entities within a service. - For more detail on this error reference: https://electrodb.dev/en/reference/errors/#join`,
    );
  ...
  it("Should require collections to be set on the same index", () => {
    ...
    database.join(entityOne);
    expect(() => database.join(entityTwo)).to.throw(
      `Collection defined on provided index "different-index-than-entity-one" does not match collection established index "(Primary Index)". Collections must be defined on the same index across all entities within a service. - For more detail on this error reference: https://electrodb.dev/en/reference/errors/#join`,
    );
    // expect(() => database.join(entityTwo)).to.throw("You cant do that");
  });
  ...
  it("Should validate the PK field matches on all added schemas", () => {
    ...
    database.join(entityOne);
    expect(() => database.join(entityTwo)).to.throw(
      `Partition Key composite attributes provided "pkz" for index "(Primary Index)" do not match established field "pk" on established index "(Primary Index)" - For more detail on this error reference: https://electrodb.dev/en/reference/errors/#join`,
    );
  });
  ...
  it("Should disallow for 'v1' construction with 'beta' entities", () => {
    ...
    expect(() => database.join(modelOne)).to.throw(
  ...
  it("Change the table name for the service and it's entities", () => {
    ...
    service.join(schema);
    let collectionParamsBefore = service.collections
      .collectionA({ prop1: "abc" })
      .params();
    let entityParamsBefore = service.entities.MyEntity.query
      .index1({ prop1: "abc" })
      .params();
    ...
    let collectionParamsAfter = service.collections
      .collectionA({ prop1: "abc" })
      .params();
    let entityParamsAfter = service.entities.MyEntity.query
      .index1({ prop1: "abc" })
      .params();
  ...
  it("Should build the correct pk and sk when the table's pk/sk are part of a collection", async () => {
    ...
    let query = database.entities.entityOne.query
      .index1({ prop1, prop2, prop3 })
      .params();
    let scan = database.entities.entityOne.scan.params();
    let get = database.entities.entityOne.get({ prop1, prop2, prop3 }).params();
    let destroy = database.entities.entityOne
      .delete({ prop1, prop2, prop3 })
      .params();
    let update = database.entities.entityOne
      .update({ prop1, prop2, prop3 })
      .set({ prop4, prop5, prop6, prop7, prop8, prop9 })
      .params();
    ...
    testKeys(
      query.ExpressionAttributeValues[":pk"],
      query.ExpressionAttributeValues[":sk1"],
    );
    testKeys(
      scan.ExpressionAttributeValues[":pk"],
      scan.ExpressionAttributeValues[":sk"],
    );
...
describe("Sub Collections", () => {
  ...
  const serviceWithMultipleCollections = new Service(
    ...
    { table: "subcollection_table" },
  );

  it("Should have all collections across all entities", () => {
    expect(
      Object.keys(serviceWithMultipleCollections.collections),
    ).to.deep.equal([
      "outercollection",
      "extracollection",
      "innercollection",
  ...
  it("Should create collections based on presence in array", () => {
    ...
    for (const test of tests) {
      let params = serviceWithMultipleCollections.collections[test.collection]({
        ...test.input,
      }).params();
  ...
  it("Should validate collections match between entities", () => {
    const tests = [
      ...
      {
        input: ["isolated", [], "collectionA", "my_entity", "collectionA"],
        output: 0,
        success: true,
      },
      {
    ...
    for (const test of tests) {
      if (test.success) {
        let result = serviceWithMultipleCollections._processSubCollections(
          ...test.input,
        );
